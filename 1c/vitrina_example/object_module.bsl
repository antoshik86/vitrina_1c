...
Процедура ВыгрузитьНаХостинг(Сервер, Порт, Пользователь, Пароль, ПутьНаСервере) Экспорт
	СообщитьПользователю("Получение данных из 1С...");
	...
	СообщитьПользователю("Сохранение catalog.json...");
	ЗаписатьФайлJSON(КаталогВыгрузки + "catalog.json", Данные);
	СообщитьПользователю("Генерация HTML-карточек товаров...");
	...
	КонецЦикла;
	СообщитьПользователю("Подключение к FTP...");
